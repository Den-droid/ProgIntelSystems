...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state: Any):
        ...
        if coords in self.corners:
            if coords not in visited_corners:
                visited_corners.append(coords)
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x_coord + dx), int(y_coord + dy)
            hits_wall = self.walls[nextx][nexty]
            if not hits_wall:
                already_visited_corners = list(visited_corners)
                next_state = (nextx, nexty)
                if next_state in self.corners:
                    if next_state not in already_visited_corners:
                        already_visited_corners.append(next_state)
                next_successor = ((next_state, already_visited_corners), action, 1)
